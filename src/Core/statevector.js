...
exports.statevector = (elemnts, theta, mu) => {
  ...
  const v = Math.sqrt(mu / p)

  // Position  vect
  const rV = multiply(r, [Math.cos(theta), Math.sin(theta), 0])

  // Velocity vector
  const k = parseInt(e) + Math.cos(theta)
  const vV = multiply(v, [-Math.sin(theta), k, 0])

  // Rotation ma
  const R313 = multiply(multiply(rotz(w), rotx(argp)), rotz(i))

  // Rotate vectors to ICF (xyz)
  const r_ = multiply(R313, rV)
  const v_ = multiply(R313, vV)
